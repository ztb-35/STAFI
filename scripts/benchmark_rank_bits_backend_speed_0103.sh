...
# Prefer CUDA/cuDNN libs bundled in the active Python env to avoid system CUDA conflicts.
...
if [ -z "$WEIGHTS_JSON" ]; then
    WEIGHTS_JSON="$(ls -1t \
      op_0103/weights_candidates_0103*.json \
      op_0103/out/weights_candidates_0103*.json 2>/dev/null | head -n 1 || true)"
fi

if [ -z "$WEIGHTS_JSON" ] || [ ! -f "$WEIGHTS_JSON" ]; then
    echo "Error: No valid weights JSON found."
    echo "Pass by arg or env, e.g.:"
    echo "  sbatch scripts/benchmark_rank_bits_backend_speed_0103.sh op_0103/out/weights_candidates_0103_gpu.json"
    exit 1
fi
