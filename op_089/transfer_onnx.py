...
def _fallback_shape(name):
    """
    针对 openpilot supercombo 常见输入的兜底形状（batch=1）
    """
    name_low = name.lower()
    if "input_imgs" in name_low or "images" in name_low or "img" in name_low:
        return (1, 12, 128, 256)  # 两帧×6通道
    if "desire" in name_low:
        return (1, 8)
    if "traffic_convention" in name_low or "traffic" in name_low or "tc" in name_low:
        return (1, 2)
    if "initial_state" in name_low or "rnn" in name_low or "state" in name_low:
        return (1, 512)
    return (1, 1)  # 最保守兜底


def _shape_from_onnx_io(io):
    """
    从 onnxruntime I/O 拿到 shape；若包含动态维度(None/str)，回填常见默认值。
    """
    dims = []
    for d in io.shape:
...
if __name__ == '__main__':
    ...
    # 2) 为每个输入生成“全 0”feed，优先用模型里带的 shape；不行再用兜底形状
    feed = {}
    torch_inputs = []
    for meta in inputs_meta:
        shp = _shape_from_onnx_io(meta)
        if shp is None:
            shp = _fallback_shape(meta.name)
        arr = np.zeros(shp, dtype=np.float32)
        feed[meta.name] = arr
        torch_inputs.append(torch.from_numpy(arr))

    # 找输出名（你之前固定为 'outputs'，这里更稳妥：若存在就用它；否则用第一个输出名）
    output_name = 'outputs'
    ...
    # 3) 运行 ONNX：全 0 输入
    onnx_res = session.run([output_name], feed)
    onnx_out = np.array(onnx_res[0])

    print("########## ONNX Model Output ##########")
    print(onnx_out.shape)
    # 以下切片区间基于 0.8.x 常见布局，如需严格以你版本为准可自行调整
    ...
    # 4) 加载 PyTorch 模型 & 从 ONNX 权重迁移（假定你已有 OpenPilotModel 与 load_weights_from_onnx）
    ...
    # 5) 保存权重
    torch.save(model.state_dict(), 'supercombo_torch_weights.pth')

    # 6）导出 onnx

    ## 准备 dummy 输入（以 openpilot supercombo 为例）
    # 用 torch 张量作为 dummy 输入，别用 numpy
    ...
    print("saved: supercombo_modified.onnx")